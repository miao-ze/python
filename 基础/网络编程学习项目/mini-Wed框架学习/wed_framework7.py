"""wed框架开发---
【先wed服务器返还模板文件，再由模板文件中的ajax请求发送请求数据，把请求数据绑定到html中相应的位置，实现前后端的分离】"""
...
def moban3():
    ...
    respond_header = [("Server","MS miao"),('Allow','GET,HEAD')]
    # 1.打开指定模块文件，读取文件中的数据
    with open("mini-wed素材/moban3.html", "r",encoding="utf-8") as file:
        file_date = file.read()
    respond_body = file_date
    #返回 响应行，响应头，响应体信息给wed服务器，再让wed服务器进行拼接，返回给浏览器
    return status,respond_header,respond_body
