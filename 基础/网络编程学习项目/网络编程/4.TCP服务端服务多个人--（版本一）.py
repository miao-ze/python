...
if __name__ == '__main__':
    ...
    '''
    设置端口号复，表示的意思是：服务端程序退出时，端口号立即释放
    1.SOL_SOCKET:表示对当前的socket（套接字）进行设置
    2.SO_REUSEADDR:表示复用端口号的选项、
    3.True：表示确定复用
    '''
    client_service.setsockopt(socket.SOL_SOCKET,socket.SO_REUSEADDR,True)
